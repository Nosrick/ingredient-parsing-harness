...
public class ItemStackHandler implements ItemHandler {
    ...
    protected void validateSlotIndex(int slot) {
        if (slot < 0 || slot >= inventory.size()){
            ExampleMod.LOGGER.error("Count not find slot " + slot + " in inventory of size " + inventory.size());
        }
    }
    ...
    public void fromTag(NbtCompound tag) {
        setSize(tag.contains("Size", 3) ? tag.getInt("Size") : inventory.size());
        NbtList itemListTag = tag.getList("Items", 10);
